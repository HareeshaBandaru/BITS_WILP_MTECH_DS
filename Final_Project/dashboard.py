...
API_DEFAULT_URL = "http://localhost:5000"
...
def render_chat_result(chat_data: Dict[str, Any]) -> None:
    ...
    cols = st.columns(3)
    cols[0].metric("Grounding", chat_data.get("grounding_score", 0.0))
    ...
    cols[1].metric("Hallucinated", str(chat_data.get("hallucinated", 0)))
    cols[2].metric("Token count", chat_data.get("token_count", 0))
...
def run_simulation_mode(use_api: bool, api_url: str) -> None:
    ...
    if st.button("Generate and process simulated chat"):
        ...
        customer_messages = [msg for role, msg in conversation if role == "customer"]
        if not customer_messages:
            st.warning("Simulator did not generate a customer message.")
            return

        st.markdown("**Generated customer messages**")
        for idx, msg in enumerate(customer_messages, start=1):
            st.write(f"{idx}. {msg}")
        ...
        if use_api:
            ...
            result = call_api_simulate(api_url, payload)
            if result.get("status") != "ok":
                st.error(result.get("message", "API request failed"))
                return
            chat_data = result["chat"]
...
def main() -> None:
    ...
    use_api = st.sidebar.checkbox("Use Flask API backend", value=False)
